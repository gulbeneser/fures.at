...
# Google API Anahtarlarınızı Ayarlama

**ÖNEMLİ:** Bu demo, düzgün çalışması için çeşitli Google API'lerini kullanır. Örnek koda dahil edilen API anahtarları yalnızca tanıtım amaçlıdır ve uygulamanın başarısız olmasına neden olabilecek kısıtlayıcı kotalara tabidir. İstikrarlı bir deneyim sağlamak ve uygulamanın tüm yeteneklerini keşfetmek için **kendi API anahtarlarınızı edinmeli ve kullanmalısınız**.

Bu uygulamayı dağıtmak için iki API anahtarına ihtiyacınız olacak:

1.  **Google Gemini API Anahtarı**: Konuşma yapay zekası için.
2.  **Google Maps Platform API Anahtarı**: Harita, yerler ve coğrafi kodlama için.

## 1. API Anahtarlarınızı Alın

Her iki anahtar için de faturalandırmanın etkin olduğu bir Google Cloud projesine ihtiyacınız olacak.

*   **Gemini API Anahtarı**: [AI Studio'dan bir Gemini API anahtarı alın](https://aistudio.google.com/app/apikey).
*   **Maps Platform API Anahtarı**: [Resmi belgelerdeki talimatları izleyerek](https://developers.google.com/maps/documentation/javascript/get-api-key) yeni bir API anahtarı oluşturun.

## 2. Gerekli API'leri Etkinleştirin

Google Cloud projenizin kontrol panelinde, "API'ler ve Hizmetler" bölümüne gidin ve aşağıdaki API'leri etkinleştirin:

*   **Geocoding API**: Adresleri coğrafi koordinatlara dönüştürür.
*   **Places API (New)**: İlgi çekici noktalar hakkında ayrıntılı bilgi alır.
*   **Maps Elevation API**: 3D harita görünümleri için rakım verilerini alır.
*   **Maps JavaScript API**: Haritayı yükler ve görüntüler.

## 3. API Anahtarlarını Netlify'da Yapılandırma

Bu uygulamayı GitHub ve Netlify kullanarak dağıtmak için:

1.  Kodu bir GitHub deposuna aktarın.
2.  GitHub deponuzu yeni bir Netlify sitesine bağlayın.
3.  Netlify sitenizin ayarlarında **"Build & deploy" > "Environment" > "Environment variables"** bölümüne gidin.
4.  Aşağıdaki iki ortam değişkenini oluşturun:

    *   `GEMINI_API_KEY`: Değer olarak Gemini API anahtarınızı girin.
    *   `MAPS_API_KEY`: Değer olarak Google Maps Platform API anahtarınızı girin.

Uygulama, dağıtım sırasında bu değişkenleri otomatik olarak kullanacaktır.

> Kendi anahtarlarınızı kullanmamanız, paylaşılan demo anahtarındaki kota sınırları nedeniyle haritanın yüklenmemesine veya özelliklerin kullanılamamasına neden olabilir.
