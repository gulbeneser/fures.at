    ...
    const AnimatedAvatar = () => (
      <>
        <div className="relative w-12 h-12">
            <div className="absolute inset-0 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 animate-pulse-slow"></div>
            <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" className="relative">
                <circle cx="24" cy="24" r="22" fill="#161B22" stroke="#30363D" strokeWidth="2"/>
                <path d="M24 18C21.7909 18 20 19.7909 20 22C20 24.2091 21.7909 26 24 26C26.2091 26 28 24.2091 28 22C28 19.7909 26.2091 18 24 18Z" fill="white" fillOpacity="0.8"/>
                <path d="M24 12C17.3726 12 12 17.3726 12 24C12 30.6274 17.3726 36 24 36C30.6274 36 36 30.6274 36 24C36 17.3726 30.6274 12 24 12ZM24 32C19.5817 32 16 28.4183 16 24C16 19.5817 19.5817 16 24 16C28.4183 16 32 19.5817 32 24C32 28.4183 28.4183 32 24 32Z" fill="white" fillOpacity="0.3"/>
            </svg>
        </div>
        <style>{`
    ...
    return (
        <>
            <div className="fixed bottom-6 right-6 z-40 no-print">
                <button onClick={() => setIsOpen(!isOpen)} className="bg-accent-color text-white p-4 rounded-full shadow-lg shadow-accent-glow hover:bg-blue-400 transition-all duration-300 transform hover:scale-110 flex items-center justify-center" aria-label={t.chatbot.title}>
                    {isOpen ? (
            ...
            <div className={`fixed bottom-24 right-6 z-50 w-full max-w-sm glass-card rounded-2xl flex flex-col h-[70vh] max-h-[600px] transition-all duration-300 ease-out no-print ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
                    ...
                    <div className="space-y-4">
                        {messages.map((msg, index) => (
                            <div key={index} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                                {msg.sender === 'bot' && <div className="w-8 h-8 rounded-full bg-slate-700 flex-shrink-0"></div>}
                                <div className={`max-w-[80%] rounded-xl px-4 py-2 shadow-sm ${msg.sender === 'user' ? 'bg-accent-color text-white' : 'bg-[#161B22] text-primary-text'}`}>
                        ...
                        {isLoading && (
                            <div className="flex items-end gap-2 justify-start">
                                ...
                                <div className="max-w-[80%] rounded-xl px-4 py-2 bg-[#161B22] text-primary-text">
                                    <div className="flex items-center space-x-2">
